...
let IsOwnerInterceptor = class IsOwnerInterceptor {
    intercept(context, next) {
        ...
        return next.handle().pipe((0, rxjs_1.tap)(todo => {
            if (!user || todo instanceof Array)
                return;
            const userId = typeof todo.owner === 'object' ? todo.owner.id : todo.owner;
            const isOwner = userId === user.id;
            if (!isOwner) {
                throw new common_1.ForbiddenException(`Todo does not belong to you`);
            }
        }));
    }
};
